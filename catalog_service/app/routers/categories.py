...
from app.crud.categories import category_crud, CATEGORY_LIMIT, CATEGORY_SKIP
from app.crud.products import PRODUCT_SKIP, PRODUCT_LIMIT
...
async def get_all_categories(skip: int = CATEGORY_SKIP, limit: int = CATEGORY_LIMIT,
...
async def get_category_products(category_id: UUID, skip: int = PRODUCT_SKIP, limit: int = PRODUCT_LIMIT,
                                session: AsyncSession = Depends(get_async_session),
                                _=Depends(permission_required('category.read'))):
